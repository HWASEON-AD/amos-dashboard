...
export async function POST() {
  ...
  if (error) {
    ...
    return NextResponse.json({ error: error.message, hint: 'sql/2026-07-03_views.sql 실행 필요' }, { status: 500 })
  }
  ...
  await Promise.all(
    (posts || []).map(async (p: { id: string; blog_url: string | null; image_host_url: string | null }) => {
      ...
      if (Object.keys(patch).length > 0) {
        await supabaseAdmin.from('amos_posts').update(patch).eq('id', p.id)
      }
    }),
  )

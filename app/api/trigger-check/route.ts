...
export async function POST(req: NextRequest) {
  ...
  // amos repo의 naver_daily_check.yml 워크플로를 master 브랜치 기준으로 dispatch
  const url = 'https://api.github.com/repos/HWASEON-AD/amos-dashboard/actions/workflows/naver_daily_check.yml/dispatches'
  ...
  try {
    const r = await fetch(url, {
      ...
      body: JSON.stringify({ ref: 'master', inputs: { post_id: post_id || '' } }),
    })

...
// - 키워드는 amos_posts에서 고른다(post_id) → 브랜드/제품/노출탭은 그 행에서 가져오므로 오타가 없다.
// - 캡처가 있다는 건 그 날 노출됐다는 뜻이므로 amos_daily_exposure에도 함께 기록한다(중복은 무시).
// - 같은 (post_id, date) 캡처가 이미 있으면 새로 만들지 않고 이미지를 갱신한다(중복 행 방지).
const BUCKET = 'amos-captures'
const TABLE = 'amos_daily_captures'
const EXPOSURE_TABLE = 'amos_daily_exposure'
const POSTS_TABLE = 'amos_posts'

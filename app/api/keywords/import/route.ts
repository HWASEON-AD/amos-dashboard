...
export async function POST(req: NextRequest) {
  ...
  // 전체 교체 모드: 기존 데이터 전부 삭제 (노출 기록도 FK cascade로 함께 삭제됨)
  if (replace) {
  ...
  type OldRow = {
    id: string; blog_url: string | null; past_urls: string | null
    hwaseon_url: string | null; past_hwaseon_urls: string | null
  }
  const existingMap = new Map<string, OldRow>()
  for (const e of existing || []) {
    existingMap.set(`${e.keyword}|||${e.product ?? ''}|||${e.brand ?? '아모스'}`, e)
  }
  ...
  for (const u of toUpdate) {
    ...
    await supabaseAdmin.from('amos_posts').update(patch).eq('id', u.id)
  }

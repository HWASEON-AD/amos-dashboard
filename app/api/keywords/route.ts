...
export async function GET() {
  ...
  // ⚠️ 1000행 제한 때문에 반드시 페이지네이션 (노출 기록은 2000건 이상)
  const exposures = await fetchAllRows<{ post_id: string; date: string }>(() =>
    supabaseAdmin.from('amos_daily_exposure').select('post_id, date'),
  )
...
export async function POST(req: NextRequest) {
  ...
  const { keyword, product, blog_url, hwaseon_url, image_host_url, tab, tab_type, brand, category, category2 } = body
